	...
	Попытка
		...
		Если НРЕГ(СокрЛП(doc_uuid)) <> "" тогда //Передан uuid документа принимаем его тип иначе ищем документ по параметрам firm_uuid, doc_date, doc_number
			...
			Если НРЕГ(СокрЛП(doc_type)) = "bill" тогда 
				...
				ТабличныйДокумент = ИнтеграцияОбщийМодуль.ПолучитьТабличныйДокументСчетНаОплату(Организация, СчетПокупателя, ДатаДокумента, НомерДокумента, prn_form);
			...
			ИначеЕсли НРЕГ(СокрЛП(doc_type)) = "sales" тогда 
				...
				ТабличныйДокумент = ИнтеграцияОбщийМодуль.ПолучитьТабличныйДокументРеализация(Организация, РеализацияТоваровУслуг, ДатаДокумента, НомерДокумента, prn_form);
			...
			ИначеЕсли НРЕГ(СокрЛП(doc_type)) = "invoice" тогда 
				...
				ТабличныйДокумент = ИнтеграцияОбщийМодуль.ПолучитьТабличныйДокументСчетФактураВыданный(Организация, СчетФактура, ДатаДокумента, НомерДокумента, prn_form);
		...
		Иначе
			...
			Если НРЕГ(СокрЛП(doc_type)) = "bill" тогда //Формируем счет на оплату клиенту
				ТабличныйДокумент = ИнтеграцияОбщийМодуль.ПолучитьТабличныйДокументСчетНаОплату(Организация, неопределено, ДатаДокумента, НомерДокумента, prn_form);
			КонецЕсли;
			
			Если НРЕГ(СокрЛП(doc_type)) = "sales" тогда //Формируем Реализацию
				ТабличныйДокумент = ИнтеграцияОбщийМодуль.ПолучитьТабличныйДокументРеализация(Организация, неопределено, ДатаДокумента, НомерДокумента, prn_form);
			КонецЕсли;
			
			Если НРЕГ(СокрЛП(doc_type)) = "invoice" тогда //Формируем Счет фактуру выданную
				ТабличныйДокумент = ИнтеграцияОбщийМодуль.ПолучитьТабличныйДокументСчетФактураВыданный(Организация, неопределено, ДатаДокумента, НомерДокумента, prn_form);
			КонецЕсли;
...
Функция ПолучитьСписокСвязанныхДокументов(Запрос)
	...
	Для Каждого тЭлем Из Запрос.ПараметрыЗапроса Цикл
		Если НРЕГ(СокрЛП(тЭлем.Ключ)) = "doc_type" тогда
			doc_type = СокрЛП(тЭлем.Значение);	
		ИначеЕсли НРЕГ(СокрЛП(тЭлем.Ключ)) = "doc_uuid" тогда
			doc_uuid = СокрЛП(тЭлем.Значение);
		ИначеЕсли НРЕГ(СокрЛП(тЭлем.Ключ)) = "format" тогда
	...
	Попытка
		...
		Если НРЕГ(СокрЛП(doc_uuid)) <> "" тогда //Передан uuid документа принимаем его тип иначе ищем документ по параметрам firm_uuid, doc_date, doc_number
			...
			Если НРЕГ(СокрЛП(doc_type)) = "bill" тогда 
				НовыйGUID = Новый УникальныйИдентификатор(doc_uuid);
				СчетПокупателя = Документы.СчетНаОплатуПокупателю.ПолучитьСсылку(НовыйGUID);
				ТаблицаСвязанныхДокументов = СвязанныеДокументы.ПолучитьСписокСвязанныхДокументов(СчетПокупателя);
			...
			ИначеЕсли НРЕГ(СокрЛП(doc_type)) = "sales" тогда 
				НовыйGUID = Новый УникальныйИдентификатор(doc_uuid);
				РеализацияТоваровУслуг = Документы.РеализацияТоваровУслуг.ПолучитьСсылку(НовыйGUID);
				ТаблицаСвязанныхДокументов = СвязанныеДокументы.ПолучитьСписокСвязанныхДокументов(РеализацияТоваровУслуг);
