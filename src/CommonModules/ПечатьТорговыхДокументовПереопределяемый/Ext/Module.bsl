...
Функция ПечатьТОРГ12(СведенияТОРГ12, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_ТОРГ12_Переопределяемый");
...
Функция ПечатьСчетаНаОплату(СведенияСчетаНаОплату, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СведенияОДокументе Из СведенияСчетаНаОплату Цикл
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_СчетЗаказ_Переопределяемый");
		...
		Если СведенияОПокупателе.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ФизическоеЛицо Тогда
			ДвоичныеДанныеQRКода = ПечатьТорговыхДокументов.QRКодРеквизитыПлатежа(СведенияОДокументе, ДанныеПечатиЗаголовокСчета);
		КонецЕсли; 
		...
		Иначе
			...
			ЗаполнитьЗначенияСвойств(ДанныеПечатиИтоговНДС, 
				ПечатьТорговыхДокументов.ДанныеПечатиИтоговНДС(СуммаНДС, СтавкиНДС, СведенияОДокументе.СуммаВключаетНДС, ":"));
		КонецЕсли;
...
Функция ТабличныйДокументУПД(Макет, МассивОбъектов, ОбъектыПечати, ТабДокумент, ТекстЗапросаДокументам, ТолькоПередаточныйДокумент, ПараметрыПечати)
	...
	ИначеЕсли ТолькоПередаточныйДокумент Тогда
		ДанныеУниверсальныхПередаточныхДокументов = УчетНДС.ПолучитьДанныеДляПечатиУниверсальногоПередаточногоДокумента(
...
Функция ПечатьУниверсальныхПередаточныхДокументов(МассивОбъектов, ОбъектыПечати, ТекстЗапросаДокументам, ТолькоПередаточныйДокумент = Ложь, ТабДокумент = Неопределено, ПараметрыПечати) Экспорт
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("ОбщийМакет.ПФ_MXL_УниверсальныйПередаточныйДокумент_Переопределяемый");
