...
&НаСервере
Функция ОбновитьНаСервере(Документ, Продукт, СтатьяРасходовУУ, ВидДокумента)
	...
	Попытка
		...
		Док.Продукт = Продукт;
		Док.СтатьяРасходовУУ = СтатьяРасходовУУ;
...
Процедура Обновить(Команда)
	...
	Для Каждого СтрокаТЧ из Объект.СписокДокументов Цикл
		...
		ДокументОбновлен = ОбновитьНаСервере(СтрокаТЧ.Документ, Объект.Продукт, Объект.СтатьяРасходовУУ, Объект.ВидДокумента);
		Если ДокументОбновлен тогда
			СтрокаТЧ.Продукт = Объект.Продукт;
			СтрокаТЧ.СтатьяРасходовУУ = Объект.СтатьяРасходовУУ;
