...
&НаСервере
Процедура РегистрацияОбъектовНаСервере(тМассив)
	МассивОбъектовOData = Новый Массив;
	Для Каждого ЭлементМассива из тМассив цикл
		МассивОбъектовOData.Добавить(Метаданные.НайтиПоПолномуИмени(ЭлементМассива));
	КонецЦикла;	                                                                     
	УстановитьСоставСтандартногоИнтерфейсаOData(МассивОбъектовOData);
	Сообщить("Публикация закончена!");
КонецПроцедуры
...
Процедура РегистрацияОбъектов(Команда)
	...
	//тМассив.Добавить(Метаданные.Справочники.Сотрудники);
	//тМассив.Добавить(Метаданные.Справочники.ФизическиеЛица);
	//тМассив.Добавить(Метаданные.Справочники._ВыгрузкаРеестраНаЗачислениеДенежныхСредствВТБПрисоединенныеФайлы);
	//тМассив.Добавить(Метаданные.РегистрыБухгалтерии.Хозрасчетный);
	//тМассив.Добавить(Метаданные.ПланыСчетов.Хозрасчетный);
	//тМассив.Добавить(Метаданные.ОбщиеМодули._Общий);
	//тМассив.Добавить(Метаданные.Справочники.Продукт);
	//тМассив.Добавить(Метаданные.Справочники.ТелефоныКонтрагента);
	//тМассив.Добавить(Метаданные.Справочники.БанковскиеКартыКонтрагента);

	Для Каждого ВыбранныйЭлемент из ВыбранныеОбъектыМетаданных цикл
            тМассив.Добавить(ВыбранныйЭлемент.Значение);
	КонецЦикла;	
	РегистрацияОбъектовНаСервере(тМассив);
КонецПроцедуры
